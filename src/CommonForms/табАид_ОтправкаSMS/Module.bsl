...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Организация", ЭтотОбъект.Организация) тогда
		ЭтотОбъект.Организация = Справочники.Организации.ПустаяСсылка();
	КонецЕсли;
	Если Не Параметры.Свойство("ИмяОтправителя", ЭтотОбъект.ИмяОтправителя) тогда
		ЭтотОбъект.ИмяОтправителя = "";
	КонецЕсли;
...
Процедура Отправить(Команда)
	...
	Если НЕ НастройкаОтправкиSMSВыполнена(ЭтотОбъект.Организация) Тогда
...
Процедура ОтправитьSMS()
	...
	// отправка
	РезультатОтправки = табАид_ОтправкаSMS.ОтправитьSMS(МассивНомеров, ТекстСообщения, ИмяОтправителя, ОтправлятьВТранслите);
...
Функция СтатусДоставки(ИдентификаторСообщения)
	...
	РезультатДоставки.Статус = табАид_ОтправкаSMS.СтатусДоставки(ИдентификаторСообщения);
...
&НаСервереБезКонтекста
Функция НастройкаОтправкиSMSВыполнена(Организация)
	
	Возврат табАид_ОтправкаSMS.НастройкаОтправкиSMSВыполнена(Организация);
