...
Функция табАид_ПараметрыШаблона(Шаблон)
	...
	Если ЗначениеЗаполнено(Результат.ПолноеИмяТипаНазначения) 
		И НЕ ШаблоныСообщенийСлужебный.ОбъектЯвляетсяПредметомШаблона(Результат.ПолноеИмяТипаНазначения) Тогда
...
Функция табАид_ЗаполнитьПараметрыСообщения(ПараметрыШаблона, ПараметрыТекстаСообщения, ПараметрыОтправки)
			...
			Если КлючПараметра = ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров() Тогда
				...
				Если ТипЗнч(ПроизвольныеРеквизиты ) = Тип("Соответствие") Тогда
					Для Каждого ПроизвольныйРеквизит Из ПроизвольныеРеквизиты Цикл
						ОписаниеПараметра = ШаблоныСообщенийСлужебный.ИмяПараметраБезСтрокиФормата(ПроизвольныйРеквизит.Ключ);
	...
	Если ПараметрыТекстаСообщения[ШаблоныСообщений.ИмяУзлаОбщихРеквизитов()] <> Неопределено Тогда
		ШаблоныСообщенийСлужебный.ЗаполнитьОбщиеРеквизиты(ПараметрыТекстаСообщения[ШаблоныСообщений.ИмяУзлаОбщихРеквизитов()]);
...
Функция табАид_УстановитьЗначенияРеквизитовВТекстСообщения(ПараметрыШаблона, ПараметрыТекстаСообщения, Предмет)
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
		Результат.Тема = ШаблоныСообщенийСлужебный.ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(ПараметрыШаблона.Тема, ПараметрыТекстаСообщения.Получить("табАид_ТаблицаДанных")[0].РеквизитыДляСтроки,"табАид_ВиртуальныйМенеджер.");
	КонецЕсли;
	...
	иначе
		//-табАид	
		Результат.Текст = ШаблоныСообщенийСлужебный.ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(ПараметрыШаблона.Текст, ПараметрыТекстаСообщения);
	КонецЕсли;
	Возврат Результат;
	...
	Если ЗначениеЗаполнено(ПараметрыШаблона.ПолноеИмяТипаНазначения) Тогда
		...
		Если МетаданныеОбъект <> Неопределено Тогда
			...
			Иначе
				...
				ШаблоныСообщенийСлужебный.РеквизитыПоСКД(РеквизитыОбъектаНазначения, МакетСКД, МетаданныеОбъект.Имя);
			КонецЕсли;
			...
			Если РазворачиватьСсылочныеРеквизиты Тогда
				Если РеквизитОбъектаНазначения.Тип.Типы().Количество() = 1 Тогда
					...
					Если ТипОбъекта <> Неопределено И СтрНачинаетсяС(ТипОбъекта.ПолноеИмя(), "Справочник") Тогда
						ШаблоныСообщенийСлужебный.РазвернутьРеквизит(РеквизитОбъектаНазначения.Имя, РеквизитыОбъектаНазначения);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьПочтовыйАдресДляОтправки(Знач Организация = Неопределено) Экспорт
		...
		Если ВложениеПечатнаяФорма.Статус = "ПечатнаяФорма" И ИмяПараметраСПечатнойФормойВШаблоне <> Неопределено Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
				...
				Исключение
					...
					ШаблоныСообщенийСлужебный.ИмяСобытияЖурналаРегистрации(),
...
//Функции добавлены из ШаблоныСообщенийСлужебный
	...
	Для каждого ПараметрОснования Из ПараметрыТекстаСообщения Цикл
		...
		Если СтрНачинаетсяС(ПараметрОснования.Ключ, "~Свойство") Тогда
			...
			Для каждого ПараметрСвойство Из ПараметрОснования.Значение Цикл
				Если ЗначенияСвойств <> Неопределено Тогда
					Для каждого СтрокаСвойство Из ЗначенияСвойств Цикл
						ОписаниеПараметра = ШаблоныСообщенийСлужебный.ИмяПараметраБезСтрокиФормата(ПараметрСвойство.Ключ);
...
Процедура ЗаполнитьЗначенияРеквизитовПоПараметрам(Сообщение, Знач ПараметрыТекстаСообщения, Знач ПараметрыШаблона, Предмет)
	...
	Если ЗначениеЗаполнено(ПараметрыШаблона.Макет) Тогда
		ШаблоныСообщенийСлужебный.ЗаполнитьРеквизитыПоСКД(ПараметрыТекстаСообщения, Предмет, ПараметрыШаблона);
	Иначе
...
Функция УстановитьЗначенияРеквизитовВТекстСообщения(ПараметрыШаблона, ПараметрыТекстаСообщения, Предмет)
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
		Результат.Тема = ШаблоныСообщенийСлужебный.ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(ПараметрыШаблона.Тема, ПараметрыТекстаСообщения);
	КонецЕсли;
	Результат.Текст = ШаблоныСообщенийСлужебный.ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(ПараметрыШаблона.Текст, ПараметрыТекстаСообщения);
	...
	Для каждого Реквизит Из ПараметрыТекстаСообщения Цикл
		...
		Иначе
			Если ПустаяСтрока(Реквизит.Значение) Тогда
				...
				ОписаниеРеквизита = ШаблоныСообщенийСлужебный.ИмяПараметраБезСтрокиФормата(Реквизит.Ключ);
