...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПолноеИмяТипаПараметраВводаНаОсновании.СписокВыбора.Добавить(Аид_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий(),
		Аид_ШаблоныСообщенийКлиентСервер.ОбщийПредставление());
	НастройкиШаблоновСообщений = Аид_ШаблоныСообщенийСлужебныйПовтИсп.ПриОпределенииНастроек();
	...
	Если Параметры.Ключ = Неопределено Или Параметры.Ключ.Пустая() Тогда
		...
		Если Параметры.ЗначениеКопирования = Справочники.Аид_ШаблоныСообщений.ПустаяСсылка() Тогда
	...
	//Костя
	лпСписок = Элементы.ПолноеИмяТипаПараметраВводаНаОсновании.СписокВыбора;
	лпНовыйСписок = лпСписок.Скопировать();
	лпНовыйСписок.Очистить();
	
	Индекс = лпСписок.Количество() - 1;
	Пока Индекс >=0 Цикл
		Если лпСписок[Индекс].Представление =  "АИД" Тогда 
			лпНовыйСписок.Добавить(лпСписок[Индекс].Значение,лпСписок[Индекс].Представление,,);
		КонецЕсли;
		Если  лпСписок[Индекс].Представление = "Сверка взаиморасчетов" Тогда 
			лпНовыйСписок.Добавить(лпСписок[Индекс].Значение,лпСписок[Индекс].Представление,,);
		КонецЕсли;
		Индекс = Индекс - 1;
	КонецЦикла;
	
	Элементы.ПолноеИмяТипаПараметраВводаНаОсновании.СписокВыбора.Очистить();
	
	Индекс1 = лпНовыйСписок.Количество() - 1;
	Пока Индекс1 >=0 Цикл
		Элементы.ПолноеИмяТипаПараметраВводаНаОсновании.СписокВыбора.Добавить(лпНовыйСписок[Индекс1].Значение,лпНовыйСписок[Индекс1].Представление,,);
		Индекс1 = Индекс1 - 1;
	КонецЦикла;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ПустаяСтрока(Объект.ПолноеИмяТипаПараметраВводаНаОсновании) Тогда
		Объект.Назначение = Аид_ШаблоныСообщенийКлиентСервер.ОбщийПредставление();
		Объект.ПредназначенДляВводаНаОсновании = Ложь;
		Объект.ПолноеИмяТипаПараметраВводаНаОсновании = Аид_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Объект.ПредназначенДляВводаНаОсновании Тогда
		...
		Объект.Назначение = Аид_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий();
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ИнформацияОПроверке.Успех Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Шаблон сообщения не может быть записан.';
													|en = 'Message template cannot be written.'")
	...
	ИначеЕсли ТекущийОбъект.Аид_ПредназначенДляГолосовогоСообщения Тогда
		ТекущийОбъект.ТекстШаблонаSMS = ИнформацияОПроверке.ОбычныйТекст;
		ТекущийОбъект.ФорматВложений = Неопределено;
	...
	Иначе
		...
		Если ТаблицаСоответствийИменВложенийИдентификаторам.Количество() > 0 Тогда
			...
			ДокументHTML = Аид_ШаблоныСообщенийСлужебный.ПолучитьОбъектДокументHTMLИзТекстаHTML(ТекущийОбъект.ТекстШаблонаПисьмаHTML);
			ЗаменитьИменаКартинокНаИдентификаторыПочтовыхВложенийВHTML(ДокументHTML, ТаблицаСоответствийИменВложенийИдентификаторам);
			ТекущийОбъект.ТекстШаблонаПисьмаHTML = Аид_ШаблоныСообщенийСлужебный.ПолучитьТекстHTMLИзОбъектаДокументHTML(ДокументHTML);
...
Функция ОбработатьТекстШаблона()
	...
	Если Объект.ПредназначенДляSMS Тогда
		Результат.ОбычныйТекст = 
			Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(Результат.ОбычныйТекст, ДеревоПараметровШаблона, ВариантПреобразования);
	...
	Иначе
		Если Результат.ТипТекстаПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML Тогда
			...
			Результат.HTMLТекст = Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(СтрЗаменить(Результат.HTMLТекст, "&quot;", """"), ДеревоПараметровШаблона, ВариантПреобразования);
			Результат.ОбычныйТекст = Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТелоПисьмаВHTML.ПолучитьТекст(), ДеревоПараметровШаблона, ВариантПреобразования);
		...
		Иначе
			...
			КонецЕсли;
			Результат.ОбычныйТекст = Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(Результат.ОбычныйТекст,
		...
		Результат.ТемаПисьма = Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(Результат.ТемаПисьма, ДеревоПараметровШаблона, ВариантПреобразования);
	...
	ПараметрыШаблонаИзТекста = Аид_ШаблоныСообщенийСлужебный.ПараметрыТекстаСообщения(ПроверяемыйТекст);
	
	Для каждого ПараметрШаблонаИзТекста Из ПараметрыШаблонаИзТекста Цикл
		
		ОписаниеПараметра =  Аид_ШаблоныСообщенийСлужебный.ИмяПараметраБезСтрокиФормата(ПараметрШаблонаИзТекста.Ключ);
	...
	Если ОшибочныеПараметры.Количество() > 0 Тогда
		ТекстОшибки = ?(ОшибочныеПараметры.Количество() = 1,
			НСтр("ru = 'Некорректный реквизит в шаблоне сообщения:';
				|en = 'Incorrect attribute in message template:'"),
			НСтр("ru = 'Некорректные реквизиты в шаблоне сообщения:';
				|en = 'Incorrect attributes in the message template:'"));
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.Аид_ПредназначенДляГолосовогоСообщения	 Тогда
		Возврат;
	КонецЕсли;
	...
	Пока Индекс >= 0 Цикл
		СтрокаТаблицыВложений = Аид_ШаблоныСообщений.СтрокаВложений(Вложения.Получить(Индекс));
		Если СтрокаТаблицыВложений.Статус = "ВнешнийУдаляемый" Тогда
			Если Не ЗначениеЗаполнено(СтрокаТаблицыВложений.Ссылка) Тогда
		...
		ИначеЕсли СтрокаТаблицыВложений.Статус = "ВнешнийНовый" Тогда
			...
			СсылкаНаФайл = Аид_ШаблоныСообщенийСлужебный.ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(ТекущийОбъект.Ссылка, СтрокаТаблицыВложений, ИмяФайла, 0);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" И ТипЗнч(Источник) = Тип("СправочникСсылка.Аид_ШаблоныСообщенийПрисоединенныеФайлы") Тогда
...
Процедура ПолноеИмяТипаПараметраВводаНаОснованииПриИзменении(Элемент)
	Если ПустаяСтрока(Объект.ПолноеИмяТипаПараметраВводаНаОсновании) Тогда
		Объект.ПолноеИмяТипаПараметраВводаНаОсновании = Аид_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий();
	КонецЕсли;
	Объект.ПредназначенДляВводаНаОсновании = (Объект.ПолноеИмяТипаПараметраВводаНаОсновании <> Аид_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий());
...
Процедура РеквизитыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ИспользоватьПроизвольныеПараметры Тогда
		...
		ОткрытьФорму("Справочник.Аид_ШаблоныСообщений.Форма.ПроизвольныйПараметр", ПараметрыФормы,,,,, ОповещениеОЗакрытие);
	КонецЕсли;
КонецПроцедуры
...
Процедура РеквизитыПередУдалением(Элемент, Отказ)
	Если ИспользоватьПроизвольныеПараметры Тогда
		...
		Если СтрНачинаетсяС(ТекущиеДанные.Имя, Аид_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) Тогда
			Отбор = Новый Структура("ИмяПараметра", Сред(ТекущиеДанные.Имя, СтрДлина(Аид_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) + 2));
		Иначе
...
Процедура ИзменитьВложение(Команда)
	...
	Если ТекущиеДанные.Ссылка = ПредопределенноеЗначение("Справочник.Аид_ШаблоныСообщенийПрисоединенныеФайлы.ПустаяСсылка") Тогда
		...
		ТекстВопроса = НСтр("ru = 'Свойства файла доступны только после его записи. Записать?';
							|en = 'The file properties will be available when you save the file. Do you want to save it?'");
...
Процедура СкопироватьВложение(Команда)
	...
	Если ТекущиеДанные.Ссылка = ПредопределенноеЗначение("Справочник.Аид_ШаблоныСообщенийПрисоединенныеФайлы.ПустаяСсылка") Тогда
		...
		ТекстВопроса = НСтр("ru = 'Файл возможно скопировать только после записи шаблона сообщения. Записать?';
							|en = 'You can copy the file only after the message template is written. Write?'");
		ПоказатьВопрос(ОбработчикОповещенияОЗакрытии, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура ПослеЗакрытияФормыПараметра(ОписаниеПараметра, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(ОписаниеПараметра) = Тип("Структура") Тогда
		...
		Иначе
			...
			Если СтрНачинаетсяС(Реквизит.Имя, Аид_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) Тогда
				Отбор = Новый Структура("ИмяПараметра", Сред(Реквизит.Имя, СтрДлина(Аид_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) + 2));
			Иначе
...
Процедура УстановитьВыборФормата(Знач ФорматыСохранения = Неопределено)
	...
	Если Объект.Аид_ПредназначенДляГолосовогоСообщения Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьНовыйШаблонСообщений(Знач НастройкиШаблоновСообщений)
	...
	Если ЗначениеЗаполнено(Параметры.ПолноеИмяТипаОснования)
		 И Аид_ШаблоныСообщенийСлужебный.ОбъектЯвляетсяПредметомШаблона(Параметры.ПолноеИмяТипаОснования) Тогда
	...
	Если Параметры.Основание = Неопределено Тогда
		Если ВидСообщения = "ГолосовоеСообщение" Тогда
			Объект.ПредназначенДляSMS = ложь;
			Объект.ПредназначенДляЭлектронныхПисем = Ложь;
			Объект.Аид_ПредназначенДляГолосовогоСообщения = Истина;
			//Костя
			Объект.ТипТекстаПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.ПустаяСсылка();
			//**********************
			// Выводим на форму элемент "Прощание".
			
		КонецЕсли;
	КонецЕсли;
	
	Если Параметры.ЭтоАИД Тогда
		Объект.ПолноеИмяТипаПараметраВводаНаОсновании = "Обработка.Аид_ВиртуальныйМенеджер";
		Объект.ПредназначенДляВводаНаОсновании = Истина;
		Объект.Назначение = "АИД";
		СформироватьСписокРеквизитовИПечатныхФорм();
	КонецЕсли;
...
Процедура ШаблонНаОснованииДокументаВзаимодействия()
	...
	Если Объект.ТипТекстаПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML Тогда
		...
		Сообщение = Аид_ШаблоныСообщенийСлужебный.КонструкторСообщения();
		Сообщение.Текст = Объект.ТекстШаблонаПисьмаHTML;
		Аид_ШаблоныСообщенийСлужебный.ОбработатьHTMLДляФорматированногоДокумента(ПараметрШаблона, Сообщение, Истина, ВложенияПисьма);
		СтруктураВложений = Новый Структура();
...
Процедура УстановитьТекстШаблона(ТекущийОбъект, СписокФайлов = Неопределено)
	...
	Если ТекущийОбъект.ПредназначенДляSMS Тогда
		...
		Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТекстШаблонаSMS, СписокВсехПараметров, "ПараметрыВПредставление"));
		//Костя
	ИначеЕсли ТекущийОбъект.Аид_ПредназначенДляГолосовогоСообщения  Тогда 
	...
	Иначе
		Если ТекущийОбъект.ТипТекстаПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML Тогда
			УстановитьHTMLДляФорматированногоДокумента(
				Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТекстШаблонаПисьмаHTML, СписокВсехПараметров, "ПараметрыВПредставление"), ТекущийОбъект.Ссылка, СписокФайлов);
		Иначе
			ТелоСообщенияОбычныйТекст.УстановитьТекст(
				Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТекстШаблонаПисьма, СписокВсехПараметров, "ПараметрыВПредставление"));
		КонецЕсли;
		
		Объект.ТемаПисьма = Аид_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТемаПисьма, СписокВсехПараметров, "ПараметрыВПредставление");
...
Если Объект.ПредназначенДляSMS Тогда
	...
	//+таб
	иначеЕсли Объект.Аид_ПредназначенДляГолосовогоСообщения Тогда
	...
	Элементы.ГруппаДоступ.Видимость = 
		НЕ ПараметрыДоступа("Изменение", Метаданные.Справочники.Аид_ШаблоныСообщений, "Ссылка").ОграничениеУсловием;
...
Процедура УстановитьHTMLДляФорматированногоДокумента(ТекстШаблонаПисьмаHTML, СсылкаНаТекущийОбъект, СписокФайлов = Неопределено)
	...
	Аид_ШаблоныСообщенийСлужебный.ОбработатьHTMLДляФорматированногоДокумента(ПараметрШаблона, Сообщение, Истина, СписокФайлов);
	СтруктураВложений = Новый Структура();
...
Процедура СформироватьСписокРеквизитовИПечатныхФорм()
	...
	ПараметрыШаблона = Аид_ШаблоныСообщенийСлужебный.ПараметрыШаблона(Объект);
	СведенияОШаблоне = Аид_ШаблоныСообщенийСлужебный.СведенияОШаблоне(ПараметрыШаблона);
...
Процедура ЗаполнитьДеревоРеквизитов(Приемник, Источник, ЭтоОбщиеИлиПроизвольныеРеквизиты = Неопределено)
	...
	Для Каждого СтрокаДерева Из Источник.Строки Цикл
		...
		Если ЭтоОбщиеИлиПроизвольныеРеквизиты = Неопределено Тогда
			Если СтрокаДерева.Имя = Аид_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()
				ИЛИ СтрокаДерева.Имя = Аид_ШаблоныСообщенийСлужебный.ЗаголовокОбщиеРеквизиты() Тогда
...
Функция СкопироватьВложенияИзИсточника()
	...
	ОписаниеОшибки = НСтр("ru = 'Не удалось скопировать вложение по причине: %1';
							|en = 'Cannot copy the attachment due to: %1'");
			...
			Если ПустаяСтрока(Вложение.ИДФайлаЭлектронногоПисьма) Тогда
				...
				Исключение
					...
					ЗаписатьОшибкуВЖурналРегистрации(ИмяСобытияШаблоныСообщений(), ИнформацияОбОшибке, НСтр("ru = 'Не удалось извлечь и записать присоединенный файл для копирования';
																											|en = 'Failed to extract and record the attached file for copying'"));
...
Процедура УдалитьПрисоединенныйФайл(ПрисоединенныйФайл)
	...
	ЭлементБлокировки = Блокировка.Добавить("Справочник.Аид_ШаблоныСообщенийПрисоединенныеФайлы");
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ИмяСобытия, ИнформацияОбОшибке, ТекстСобытия)
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Аид_ШаблоныСообщений,, Комментарий);
...
Функция ИмяСобытияШаблоныСообщений()
	...
	Возврат НСтр("ru = 'Шаблоны сообщений';
				|en = 'Message templates'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьШаблонПоВнешнейОбработке()
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru = 'Предмет %1 определенный в внешней обработке не найдено. Внешняя обработка не может быть подключена.';
									|en = 'The %1 object determined in the external data processor is not found. External data processor cannot be connected.'");
